...
// We will use this struct to create a list of frameworks in the next section.
// it represents our data model.
// Identifiable: Required by ForEach to uniquely identify each element.
// Hashable: Only needed if you plan to store your Framework instances in a collection like a Set, or if you need to compare them for equality.
...
// DubDub app we used enums and computed properties. Best way for now is use like below
// values wont change bc they are static.
enum MockData {
